...
RViz renders that load from `/deflecomp_sim/external_wrench_marker`, not directly from the command `WrenchStamped`. The marker starts at the current application point, is expressed in `base_link`, and points in the applied external-force direction; therefore the default payload arrow remains world-downward even when the target link rotates.
