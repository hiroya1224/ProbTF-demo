...
class Visualizer:
    ...
    def show_triplet(
        ...
        spring_model = spring_model or LinearSpringModel()
        theta_equil_true = EquilibriumSolver(robot_sim, spring_model, EquilibriumConfig(maxiter=80)).solve(
            theta_cmd_final,
            kp_true,
            theta_init=theta_cmd_final,
        )
        P_true = self.joint_positions(robot_sim, theta_equil_true)
        theta_equil_est = EquilibriumSolver(robot_est, spring_model, EquilibriumConfig(maxiter=80)).solve(
            theta_cmd_final,
            np.maximum(kp_est, 1e-8),
            theta_init=theta_cmd_final,
        )
