...
def _summary_from_result(
    ...
    optimizer_final = (
        optimizer_diagnostics.get("final", {})
        if isinstance(optimizer_diagnostics, Mapping)
        else {}
    )
    optimizer_jacobian = (
        optimizer_final.get("jacobian", {})
        if isinstance(optimizer_final, Mapping)
        else {}
    )
    return {
        "status": "completed",
        ...
        "optimizer_reported_optimality": optimizer.get("optimality") if isinstance(optimizer, Mapping) else None,
        "optimizer_final_gradient_inf_norm": optimizer_final.get("gradient_inf_norm"),
        "optimizer_gradient_tolerance_satisfied": optimizer_diagnostics.get(
            "gradient_tolerance_satisfied"
        ) if isinstance(optimizer_diagnostics, Mapping) else None,
        "optimizer_jacobian_rank": optimizer_jacobian.get("numerical_rank"),
        "optimizer_jacobian_condition_number_nonzero_subspace": optimizer_jacobian.get(
            "condition_number_nonzero_subspace"
        ),
        "optimizer_finite_difference_max_relative_error": optimizer_diagnostics.get(
...
def _ablation_diagnostic_lines(cases: Sequence[Mapping[str, Any]]) -> list[str]:
    lines = [
        "Savitzky-Golay window ablation numerical diagnostics",
        "",
        "No new plots are generated by this diagnostic summary.",
        "",
    ]
        ...
        lines.extend(
            [
                ...
                "  optimizer status={} success={} nfev={} optimality={}".format(
                    ...
                    item.get("optimizer_nfev"),
                    item.get("optimizer_reported_optimality"),
                ),
                "  final gradient inf-norm={}".format(
                    item.get("optimizer_final_gradient_inf_norm")
                ),
                "  gradient tolerance satisfied={}".format(
                    item.get("optimizer_gradient_tolerance_satisfied")
                ),
                "  Jacobian rank={} cond(nonzero)={}".format(
                    item.get("optimizer_jacobian_rank"),
                    item.get("optimizer_jacobian_condition_number_nonzero_subspace"),
                ),
...
def run(arguments: argparse.Namespace, passthrough: Sequence[str] = ()) -> int:
    ...
    payload["outputs"] = {
        "ablation_pdf": "ablation.pdf",
        "ablation_diagnostics_txt": "ablation_diagnostics.txt",
        "ablation_diagnostics_pdf": "ablation_diagnostics.pdf",
    }
